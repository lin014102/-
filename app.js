...
setInterval(() => {
  ...
  console.log(`🧹 清理完成 - 訊息ID: ${oldSize}個, 過期提醒: ${cleanedCount}個, 記憶體: ${memUsageMB}MB, DB: ${dbStatus}`);
}, 1800000); // 30分鐘執行一次
...
console.log('🐛 輸入「調試提醒」進行詳細調試');const express = require('express');
...
async function deleteTodo(userId, index) {
  ...
  if (index < 0 || index >= todos.length) {
    return `❌ 編號不正確，請輸入 1 到 ${todos.length} 之間的數 => {
