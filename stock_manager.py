...
class StockManager:
    ...
    def parse_command(self, message_text):
        ...
        if message_text == '批量設定代號':
            return {
                'type': 'batch_code_guide'
            }
        
        # 檢查缺少代號：檢查代號 或 檢查代號 爸爸
        ...
        # 設定代號：設定代號 台積電 2330
        if match := re.match(r'設定代號\s+(.+?)\s+(\w+)', message_text):
        ...
        # 股價查詢：股價查詢 台積電 或 股價 台積電
        elif match := re.match(r'(?:股價查詢|股價)\s+(.+)', message_text):
        ...
        elif match := re.match(r'(.+?)買\s+(.+?)\s+(\d+)\s+(\d+)\s+(\d{4})
...
💡 使用「檢查代號」查看哪些股票還沒設定代號"""
    ...
    def batch_set_codes(self, codes_text):
        """批量設定股票代號"""
        ...
        for line in lines:
            ...
            parts = line.split()
            if len(parts) >= 2:
                stock_name = ' '.join(parts[:-1])  # 股票名稱可能有空格
                stock_code = parts[-1]
                self.stock_data['stock_codes'][stock_name] = stock_code
                results.append(f"✅ {stock_name} → {stock_code}")
            else:
                results.append(f"❌ 格式錯誤：{line}")
    ...
    def get_missing_stock_codes(self, account_name=None):
        ...
        for acc_name, account in accounts_to_check.items():
            ...
            if account_value > 0:
                ...
                account_pnl_percent = (account_pnl / account_cost) * 100
                account_pnl_text = f"🟢 +{account_pnl:,.0f}元 (+{account_pnl_percent:.1f}%)" if account_pnl > 0 else f"🔴 {account_pnl:,.0f}元 ({account_pnl_percent:.1f}%)"
                
                result += f"   💼 帳戶投資成本：{account_cost:,}元\n"
                result += f"   💎 帳戶投資現值：{account_value:,}元\n"
                result += f"   💹 帳戶未實現損益：{account_pnl_text}\n\n"
...
if __name__ == "__main__":
    ...
    print(sm.get_all_accounts_summary()), message_text):
        ...
        elif match := re.match(r'(.+?)賣\s+(.+?)\s+(\d+)\s+(\d+)\s+(\d{4})
